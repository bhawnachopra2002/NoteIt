...
'''A set of tests to check the update_task_view.'''
...
class AllTodoViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(reverse('update-task', args=[self.task1.id]))
        self.assertRedirects(response, '/accounts/login/?next=/todo/update/'+str(self.task1.id))
    ...
    def test_view_url_exists_at_desired_location(self):
        ...
        response = self.client.get('/todo/update/'+str(self.task1.id))
        self.assertEqual(response.status_code, 200)

    # Test to check if the correct template is being rendered at logged in user's screen
    def test_logged_in_user_sees_edit_todo_template(self):
        self.client.login(username='testuser1', password='12test12')
        response = self.client.get(reverse('update-task', args=[self.task1.id]))
